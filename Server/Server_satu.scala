...
class HandleClient_satu(sock:Socket) extends Thread{
  override def run(){
    var strtam = ""
...
class myServer_satu(portnumber:Int){
  def startServer(){
    ...
    while(true){
      ...
      if(sock.isConnected){
        ...
        val thandle=new HandleClient_satu(sock)
